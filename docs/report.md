...
随着 AI 技术的快速发展，2024-2026 年间大语言模型（LLM）能力取得了突飞猛进的进步。以 ChatGPT、DeepSeek、MiniMax 为代表的 AI 模型，在文本理解、内容摘要和自然语言生成方面展现出了惊人的能力。然而，将这些 AI 能力有机地整合到社交平台中，真正为用户创造价值——而非仅仅作为"演示功能"——仍然是一个具有挑战性的工程课题。如何在适当的场景触发 AI、如何处理 AI 调用的延迟和成本、如何将 AI 输出与用户生成内容自然融合，这些都是我们在项目设计和实现中需要认真考虑的问题。
...
第四，**集成 AI 智能总结能力**。接入 DeepSeek（原 MiniMax）大模型 API，为笔记内容自动生成 100 字以内的精炼摘要。AI 总结功能采用缓存机制，已生成的总结会被持久化存储，避免重复调用 API 产生不必要的费用。用户只需在笔记详情页点击 AI 按钮，即可在几秒内获得笔记的核心要点。
...
第六，**实现通知系统**。当用户的笔记被点赞、收藏、评论，或其评论被回复，以及被其他用户关注时，系统自动生成通知记录。用户可以在通知页面按类型筛选（点赞、评论、关注等），实时了解社区互动动态。

第七，**完成响应式移动端适配**。采用 Vant 4 移动端组件库 + CSS Media Query 实现桌面端与移动端的双端适配。桌面端采用左侧边栏导航 + 多列瀑布流布局（3-5 列，单列固定 210px），移动端切换为底部 TabBar 导航 + 双列自适应瀑布流。帖子详情在桌面端以弹窗形式展示（左右布局，1000x680px），移动端则全屏展示（上下布局）。

第八，**建立完善的软件工程质量体系**。覆盖 Git 分支策略（GitHub Flow）、代码审查规范（PR Review）、单元测试（Vitest + JUnit）、CI/CD 自动化流水线（GitHub Actions）、Docker 容器化部署、云平台部署（Vercel + Render）、结构化日志（Logback JSON）、健康检查监控（Spring Boot Actuator + Micrometer）以及 API 文档自动生成（Swagger / OpenAPI）等完整的软件工程实践环节。
...
| 用户系统 | 个人主页（查看他人发布的笔记） | ✅ 已完成 | 动态路由 + 数据聚合 |
...
| 互动系统 | 点赞/取消点赞 | ✅ 已完成 | Toggle API + 唯一约束 |
| 互动系统 | 收藏/取消收藏 | ✅ 已完成 | Toggle API + 唯一约束 |
| 互动系统 | 评论（支持二级楼中楼回复） | ✅ 已完成 | parent_id 层级结构 |
| 互动系统 | 评论删除 | ✅ 已完成 | 用户权限校验 |
| 互动系统 | 关注/取消关注 | ✅ 已完成 | follows 表 + 双向查询 |
| 通知系统 | 点赞/收藏/评论/回复/关注通知 | ✅ 已完成 | 类型筛选 + 已读标记 |
...
| 响应式 | 底部 TabBar 导航（移动端） | ✅ 已完成 | Vant 4 van-tabbar |
...
- **可用性需求**：响应式布局覆盖 320px-4K 全尺寸屏幕；支持 Chrome、Edge、Firefox、Safari 等主流浏览器；提供数据初始化机制（DataInitializer），安装后即可获得完整的测试数据体验；Swagger API 文档自动生成，接口可在线调试。

- **安全性需求**：密码 BCrypt 加密存储，防止明文泄露；JWT Token 无状态认证，有效期 24 小时；文件上传类型白名单校验，防止恶意文件注入；JPA Repository 参数化查询，防止 SQL 注入攻击；跨域 CORS 严格配置，仅允许可信来源访问；敏感配置分离管理（application-secrets.properties），排除在版本控制之外。

- **可维护性需求**：前后端代码分离，职责清晰；后端采用经典三层架构（Controller → Service → Repository），每层职责单一；前端采用 Composition API + 组合式函数（Composables），逻辑复用性高；完整的 Swagger / OpenAPI 文档，接口描述与代码同步；结构化 JSON 日志输出，便于日志分析工具解析。

- **可测试性需求**：前端 215 个测试用例全部通过，语句覆盖率 81.80%，分支覆盖率 82.20%；后端 9 个测试文件覆盖核心业务逻辑和 API 接口；CI/CD 流水线自动运行测试，确保每次提交的质量。
...
Vant 4 是专为移动端设计的 Vue 3 组件库，提供了 TabBar（底部导航栏）、Popup（弹出层）、ActionSheet（上拉菜单）、ImagePreview（图片预览）、Uploader（文件上传）等 70 余个高质量组件。Vant 4 的组件设计充分考虑了移动端的手势交互和屏幕适配，其轻量化设计（按需引入）也能有效控制打包体积。配合 unplugin-vue-components 自动导入插件，开发者无需手动注册组件，在模板中直接使用即可自动完成按需引入和 Tree Shaking。
...
Spring Data JPA 作为 ORM 框架，通过接口命名约定即可自动生成查询方法，无需手写 SQL。例如，`findByUsername(String username)` 方法名即可自动生成对应的 SELECT 查询。JPA 的 `ddl-auto=update` 策略会根据实体类的变更自动更新数据库表结构，在开发阶段极大简化了数据库迁移工作。此外，JPA 的延迟加载（Lazy Fetching）和级联操作（Cascade）也有效避免了 N+1 查询问题和数据不一致性。
...
项目最初选用 MiniMax（前身为火山引擎豆包）大模型，其 M2.1 模型在中文内容理解和生成任务上表现优异。后期切换到 DeepSeek（deepseek-v4-flash），主要基于以下考量：DeepSeek 的 API 定价更具竞争力（输入 1 元/百万 token，输出 2 元/百万 token），且模型在长文本理解、摘要生成和问答能力方面经过大量中国互联网数据的训练，对中文社交内容的处理精度更高。

**部署方案选型分析：**

前端部署于 Vercel 平台，利用其全球 CDN 节点和自动 HTTPS，实现快速页面加载。后端部署于 Render 平台，提供免费层级的 Java 应用托管服务，支持 Docker 容器化部署。MySQL 数据库使用本地宿主机实例，避免云数据库的额外费用。
...
| 陈熠恒 | 2312190613 | 54 | 前端开发 / CI/CD / UI 设计 | CI/CD 流水线配置（GitHub Actions）、ESLint 配置、前端测试覆盖率提升（81.80%）、前端核心组件开发、UI 交互设计、语音搜索前端实现、Codecov 集成 |

各成员在项目各阶段的参与深度如下所示。第一阶段（项目初始化与基础搭建，第 1-2 周），林忠阳负责后端项目骨架搭建、数据库设计、Maven 配置和项目初始化脚本编写；林烨澄负责前端 Vue 3 + Vite 项目创建、路由配置、Axios/Fetch API 封装和全局样式系统建立；陈熠恒负责 ESLint 配置、CI/CD 工作流设计和前端测试框架搭建。此阶段三人协作完成了前后端开发环境的统一搭建和 Git 仓库的规范化设置。
...
第五阶段（AI 集成与部署，第 12-15 周），林忠阳负责 DeepSeek API 接入、AI 总结缓存机制、Docker Compose 编排和 Render 云平台部署配置；林烨澄负责 AI 总结前端组件和交互优化；陈熠恒负责 CI/CD 流水线优化、测试覆盖率提升和 Codecov 集成。在最终阶段，三人共同进行了全面的系统集成测试、文档撰写和项目演示准备。
...
我们的分支策略具体如下：**main 分支**是唯一的长期分支，始终保持可部署状态。任何推送到 main 分支的代码都必须通过 CI/CD 流水线的全部检查（包括代码编译、ESLint 检查、单元测试、测试覆盖率验证），确保主分支代码的质量稳定性。main 分支配置了分支保护规则（Branch Protection Rule），禁止直接推送（Direct Push），所有变更必须通过 Pull Request（PR）合并。
...
分支保护规则配置了以下质量门禁：要求至少 1 名团队成员进行 Code Review 并通过（Approve）；要求 CI/CD 所有 Job 必须通过（绿色状态）；禁止直接推送到 main 分支；要求分支在合并前必须与 main 分支保持同步（避免合并冲突）。
...
| 重构 | `refactor:` | 代码重构 | `refactor: 切换AI供应商从MiniMax到DeepSeek` |
...
- `67d6cb9` — `refactor: 切换AI供应商从MiniMax到DeepSeek`：这是一个技术决策型的重构提交。团队基于 API 定价、响应速度和模型质量等多维度评估，决定将 AI 供应商从 MiniMax 切换到 DeepSeek，涉及后端 AI 服务层、配置文件、API Key 管理等多个模块的协调变更。

- `5e475fc` — `chore: 添加 Vercel + Render 部署配置`：该提交添加了前后端在 Vercel 和 Render 云平台上的部署配置文件，包括 Vercel 的 rewrite 规则（将 `/api/*` 请求代理到 Render 后端）和 Render 的构建脚本。
...
项目从 2026 年 3 月 11 日启动至 2026 年 6 月 3 日，历时约 15 周（对应课程学期周期），共计完成 **250 次提交**，覆盖了从项目初始化到云平台部署的完整开发流程。

**各成员提交统计：**

| 成员 | Git 用户名 | 提交次数 | 占比 | 主要贡献领域 |
|------|-----------|----------|------|-------------|
| 林忠阳 | yclin30 | 100 | 40% | 后端架构、AI 集成、Docker、监控 |
| 林烨澄 | lzy11123 | 94 | 38% | 前端开发、测试、安全、UI |
| 陈熠恒 | chenyiheng111 | 54 | 22% | CI/CD、前端测试、ESLint、UI |
| 其他 | Lin Yecheng | 2 | 1% | 文档补充 |
...
- 2026-04-20：AI 智能总结功能上线（MiniMax → DeepSeek 切换）
- 2026-05-05：CI/CD 流水线 + ESLint + 前端测试覆盖率达标
- 2026-05-15：Docker 容器化部署完成
- 2026-05-25：Vercel + Render 云平台部署成功
...
- **PR #6：CI/CD 与测试** — GitHub Actions 工作流配置、ESLint 规则配置、前端测试用例补齐（达到 215 个用例，覆盖率 81.80%）、Codecov 覆盖率上传、CI 通过标准设定（Lint 零警告 + 覆盖率 >= 60% + 全部测试通过）。

- **PR #7：Docker 与云部署** — Dockerfile 多阶段构建编写、Docker Compose 编排（backend + frontend + 宿主机 MySQL）、Nginx 反向代理配置（/api → backend:8080，/uploads → backend:8080）、Vercel rewrite 规则（/api/* → Render 后端）、Render 部署配置。
...
- **PR #7：Docker 与云部署** — Dockerfile 编写、Docker Compose 编排、Nginx 反向代理配置、Vercel rewrite 规则、Render 部署配置、镜像构建与推送。
...
| Swagger UI | API 文档浏览与调试 | 自动生成 + 在线测试 |
...
场景举例：小王周末做了一道精致的红烧肉，拍了 5 张照片。他打开 Essencity 发布页面，一次性上传 5 张图片，输入标题"超好吃的红烧肉做法"，在正文中详细写下了材料和步骤（约 300 字）。他选择了"美食"标签，然后点击发布。几分钟后，收到了 3 个点赞通知和 1 条评论"跟着做了，真的好吃"。
...
| 互动通知 | 手机 | 4G | 查看点赞/评论通知 | 实时推送 |

**核心使用流程设计：**

我们基于用户画像设计了三条核心使用流程。第一条是内容发布流程：用户登录 → 点击底部"发布"按钮 → 选择图片（1-9 张）→ 填写标题和正文 → 选择分类标签 → 点击发布 → 返回首页查看新发布的内容。第二条是内容消费流程：打开首页 → 瀑布流浏览笔记卡片（封面大图 + 标题 + 作者信息）→ 点击感兴趣的笔记 → 查看详情（大图 + 全文 + 标签 + 点赞/收藏/评论数据）→ 可选择点赞、收藏、评论或点击 AI 总结。第三条是社交互动流程：查看通知列表 → 筛选通知类型（评论/点赞/关注）→ 点击通知跳转到对应的笔记或评论 → 回复评论或关注用户。
...
第一，**首页视图（Discovery）**。顶部为带有品牌 Logo 的搜索栏（桌面端居中展示，移动端靠左简化），搜索栏集成语音搜索按钮（麦克风图标）。搜索栏下方为分类标签横滑栏（10 个标签：推荐、美食、穿搭、彩妆、影视、职场、情感、家居、游戏、旅行、健身），当前选中的标签高亮为品牌红色。主体区域为瀑布流布局（MasonryGrid），每张笔记卡片包含封面大图（3:4 竖版比例）、标题文字、底部栏（作者头像 + 昵称 + 点赞数图标）。桌面端左侧为固定 240px 的导航侧边栏，移动端底部为 Vant TabBar（首页、发布、通知、我四个图标按钮）。
...
第四，**通知页面（NotificationPage）**。顶部为通知类型筛选栏（全部、评论和回复、赞和收藏、新增关注四个 Tab）。主体为通知列表，每条通知包含：左侧触发用户的头像（圆形）、中间通知文本（模板化展示："XXX 赞了你的笔记"、"XXX 评论了你的笔记：内容"、"XXX 开始关注你了"）、右侧时间信息（相对时间如"3 分钟前"、"1 小时前"）。已读和未读通知通过背景色深浅区分。
...
第六，**登录/注册弹窗（AuthModal）**。居中模态弹窗（桌面端 400px 宽，移动端 90% 屏宽），包含用户名输入框、密码输入框、登录按钮、注册切换链接。注册模式增加昵称输入框。输入框使用 Vant 的 `van-field` 组件，带有图标前缀和清除按钮。底部提示文字"测试账号：xiaohong / 123456"。
...
第三个是"减少认知负荷"原则——配色方案极度克制，仅使用品牌红色（#FF2442）作为唯一的强调色，其余全部使用中性灰色系（#333、#666、#999）。单一强调色的好处是：用户的注意力自然集中在唯一的彩色元素上——无论是点赞按钮、标签高亮、发布按钮还是未读通知标记，红色就像"视觉路标"一样引导用户完成核心操作。如果使用多种颜色（蓝色链接、绿色按钮、橙色标签），用户需要额外花费认知资源去区分不同颜色的语义。
...
**加载状态设计**方面，图片加载过程中显示灰色占位骨架屏（Skeleton Screen），骨架屏的形状与最终内容一致（矩形图片区域 + 两行文字区域），使用 CSS `@keyframes` 动画（从左到右的 shimmer 光泽扫过效果），给用户"正在加载中"的明确反馈。数据获取过程中显示 Vant Loading 组件（居中旋转图标 + "加载中..."文字）。空状态使用专门的空状态组件（EmptyState），包含一个图标（如空文件夹）和引导文字（如"还没有发布任何笔记~"）。
...
选择前后端分离架构而非传统的服务端渲染（SSR，如 Thymeleaf 模板引擎）方案，有以下几点关键考量：首先，分离架构支持前后端独立部署和独立扩展——前端可以作为静态资源部署到 CDN（如 Vercel），后端可以部署到应用托管平台（如 Render），两者通过 API 通信，部署灵活性大大提升。其次，Vue 3 的响应式系统在客户端运行，可以提供比服务端渲染更流畅的交互体验——点赞、收藏等操作的乐观更新策略只有在前端有完整状态管理能力时才能实现。第三，前后端分离后，前端开发者无需了解 Java 和后端框架，后端开发者无需了解 Vue 和 CSS，团队协作效率显著提升。
...
第二，**API 层（API Layer）**。前端的所有 HTTP 请求通过 `src/api/index.js` 和 `src/api/ai.js` 两个模块统一管理。API 层使用浏览器原生 Fetch API，封装了以下通用逻辑：请求头的自动组装（Content-Type、Authorization Token）、响应状态码的统一处理（401 自动跳转登录、4xx/5xx 错误提示）、图片 URL 的智能处理（相对路径自动拼接 `/api` 前缀）。

第三，**状态管理层（State Management Layer）**。项目没有引入 Vuex 或 Pinia 等状态管理库，而是直接使用 Vue 3 的 `ref` 和 `reactive` API 在组件中管理本地状态。这是基于项目规模和复杂度做出的务实决策——对于 15 个组件的单页面应用，引入全局状态管理库的收益（跨组件状态共享便利）小于成本（额外的概念学习和代码开销）。需要跨组件共享的状态（当前用户信息、登录态）通过 App.vue 的 `provide` 注入。
...
**Controller 层（控制器层）**负责接收 HTTP 请求、参数校验、调用 Service 层并返回统一响应格式。在 Essencity 中，共有 6 个核心控制器：`AuthController` 处理用户注册、登录、资料更新、关注/取消关注等认证和社交关系操作；`PostController` 处理帖子的 CRUD、列表查询、点赞、收藏、文件上传和用户统计数据查询；`AIController` 处理 AI 总结生成和查询（先查缓存，再调 API）；`FileController` 处理通用文件上传和测试端点；`NotificationController` 处理通知的查询、筛选和已读标记；还有一个未在文档中列出的控制器处理评论相关操作。

**Service 层（服务层）**是业务逻辑的核心，负责数据校验、事务管理（通过 `@Transactional` 注解）、调用 Repository 层和调用外部服务（如 DeepSeek API）。关键的 Service 包括：`PostService` 管理帖子全生命周期并实现了 AI 总结缓存和帖子问答功能；`AuthService` 处理用户注册时的密码加密、登录时的密码验证和 JWT Token 生成；`AIService` 封装了对 DeepSeek API 的 HTTP 调用逻辑、Prompt 构建和响应解析；`FileService` 处理文件保存、文件名校验和文件类型白名单过滤；`NotificationService` 在点赞、收藏、评论、关注等事件触发时自动创建通知记录。

**Repository 层（数据访问层）**基于 Spring Data JPA，通过继承 `JpaRepository` 接口自动获得 CRUD 能力，通过方法命名约定定义自定义查询。关键的自定义查询方法包括：`PostRepository.findAllByOrderByCreatedAtDesc()` 按创建时间倒序获取帖子列表；`LikeRepository` 和 `CollectionRepository` 的 `findByUserAndPost` 方法用于检查是否已点赞/收藏；`CommentRepository` 提供按帖子 ID 查询评论列表和按父评论 ID 查询子回复；`NotificationRepository` 提供按用户 ID 和类型筛选通知的功能。

后端业务流程中一个典型的事务性操作是"用户点赞帖子"：首先 `Controller` 接收请求参数（用户 ID、帖子 ID），调用 `Service` 层的 `toggleLike()` 方法；`Service` 首先通过 `Repository` 查询该用户是否已对同一帖子点赞（利用唯一约束检查），如果未点赞则创建 Like 实体并保存（`likeRepository.save()`），同时创建一条点赞通知（`notificationService.create()` 向帖子作者通知"XXX 赞了你的笔记"），如果已点赞则删除对应的 Like 记录。整个过程在 `@Transactional` 注解保护下运行，确保原子性——通知创建失败不会导致点赞记录的残留。
...
**数据库事务管理**方面，Spring 的 `@Transactional` 注解在 Service 层方法上声明事务边界。默认的事务传播行为是 REQUIRED——如果当前已有事务则加入，如果没有则创建新事务。在同时操作多张表（如发布帖子需要同时插入 posts 记录和更新 tags）的场景中，事务保证了两张表的操作要么全部成功、要么全部回滚，杜绝了数据不一致的可能性。
...
REST 是当前 Web API 的事实标准，团队成员都具备 RESTful API 的开发和使用经验。GraphQL 虽然在前端灵活性方面具有优势（客户端可以精确指定需要哪些字段，避免 Over-fetching 和 Under-fetching），但其引入的额外复杂度（Schema 定义、Resolver 编写、N+1 查询问题的 DataLoader 解决方案、缓存策略重构）在我们的项目规模下是不值得的。REST 配合 Swagger 自动生成文档，已经足够满足团队的 API 开发和协作需求。
...
AI API 调用是系统中最昂贵的操作（时间约 2-5s，费用按 Token 计费）。对于缓存策略，我们最终选择了将 AI 生成结果直接存储到 posts 表的 ai_summary 字段中（数据库持久化），而非使用 Redis 或应用内存缓存。这一决策的考量是：笔记内容发布后基本不会修改，因此其 AI 总结也是不变的——缓存永远不会失效，不存在缓存一致性（Cache Invalidation）问题——这正好是最适合持久化缓存的场景。数据库查询在本地 MySQL 中仅需 < 10ms（利用主键索引），与 Redis 的 < 1ms 相比差距极小，对用户体验无感知差异。更重要的是，这避免了引入额外的 Redis 服务进程——对于学生项目来说，少一个中间件就意味着少一个可能出错的组件、少一份运维负担。如果未来系统需要支持高并发场景（如 1000+ QPS），可以非常容易地将缓存层从数据库迁移到 Redis——因为缓存逻辑已经封装在 AIService 的 generateSummary 方法中，更换底层存储只需修改这一处代码。
...
| POST | /posts/{id}/like | 切换点赞 | Toggle 模式：有则取消，无则创建 |
| POST | /posts/{id}/collect | 切换收藏 | Toggle 模式 |
...
错误响应体的格式为 `{"code": 401, "message": "Unauthorized", "data": null}`，前端根据 `code` 字段决定错误处理策略——401 触发登录弹窗，4xx 显示错误提示信息，5xx 显示"服务器开小差了，请稍后再试"。
...
**用户注册（POST /auth/register）：**接收用户名、密码、昵称三个参数。后端首先检查用户名是否已被注册（通过 users 表的 UNIQUE 约束 + Repository 的 `existsByUsername` 方法），如果未注册则使用 BCrypt 加密密码后存储到数据库，同时自动生成 JWT Token 并返回用户信息，实现"注册即登录"的流畅体验。
...
**关注/取消关注（POST /auth/follow 和 POST /auth/unfollow）：**接收 followerId（关注者）和 followingId（被关注者）参数。follow 接口在 follows 表中创建一条关注记录（利用唯一约束防止重复关注），并同时创建一条关注通知（向被关注者发送"XXX 开始关注你了"）。unfollow 接口删除对应的关注记录。两个接口均需要 JWT 认证。
...
**帖子列表查询（GET /posts）：**这是最重要的数据接口之一，支持三个可选查询参数：title（关键词模糊搜索，用于搜索功能）、tag（分类筛选，如"美食"、"穿搭"等）、page 和 size（分页参数）。未传参数时默认按创建时间倒序返回所有帖子。返回数据为 JSON 数组，每个元素包含帖子 id、title、description、type、url、coverUrl、tag、likeCount、collectionCount、createdAt 以及嵌套的 author 对象（id、nickname、avatar）。

**帖子创建（POST /posts）：**接收 title、description、type（image/video/article）、url、coverUrl、author_id、tag 等字段。后端在 Service 层对请求数据进行校验（标题不为空、类型值合法、作者 ID 存在），校验通过后创建 Post 实体并保存。type 字段支持 video（视频）、image（图片）、article（纯文本文章）三种类型。

**帖子详情（GET /posts/{id}）：**根据帖子 ID 返回完整的帖子信息，包括 title、description、type、url、coverUrl、videoUrl、imageUrl、imageUrls（多图列表）、author（嵌套用户对象）、authorId、authorNickname、authorAvatar、likeCount、collectionCount 和 createdAt。该接口同时返回帖子的关联信息，包括该帖子的评论列表（通过 `/posts/{id}/comments` 获取）和 AI 总结内容（通过帖子详情中的 aiSummary 字段）。

**文件上传（POST /posts/upload）：**接收 multipart/form-data 格式的媒体文件，后端进行文件类型校验（支持 mp4、mov、jpg、png、webp）、文件大小校验（视频最大 20GB，图片最大 20MB）后，将文件保存到 uploads 目录并返回可访问的文件 URL。

**点赞操作（POST /posts/{id}/like）：**这是一个 Toggle 接口——同一用户对同一帖子重复调用该接口，第一次创建点赞记录，第二次删除点赞记录。这种设计简化了前端逻辑，前端无需维护"已点赞"状态，直接调用同一个接口即可。接口返回当前点赞状态（"liked"）和最新点赞数。

**收藏操作（POST /posts/{id}/collect）：**采用与点赞相同的 Toggle 设计模式。

**用户帖子统计（GET /posts/user/{userId}/stats）：**返回指定用户的帖子数量、获得的总点赞数、获得的总收藏数，用于个人主页的统计数据显示。
...
**通知列表（GET /notifications）：**接收 userId 和 type 参数。type 可选值为 comments（评论和回复）、likes（赞和收藏）、follows（新增关注），不传则返回所有类型。返回的通知数组包含 type（LIKE/COLLECTION/COMMENT/REPLY/FOLLOW）、actor（触发用户信息）、postId、commentId、content、isRead、createdAt 等字段。

**标记已读（PUT /notifications/{id}/read）：**将指定通知的 isRead 字段设置为 true。

### 5.2.5 AI 相关接口

**AI 总结（POST /ai/summarize）：**这是 AI 功能的核心接口。接收 postId、title、content 参数。后端首先检查该帖子的 aiSummary 字段是否已有缓存，如果有缓存则直接返回（避免重复调用 AI API 产生费用）。如果无缓存，则调用 DeepSeek API，使用精心设计的 Prompt 模板请求生成 100 字以内的内容摘要，保存到数据库的 aiSummary 字段中，然后返回给前端。
...
**权限控制层面：**项目采用了"白名单 + 注解"的权限控制策略。在 SecurityConfig 中配置了公开接口白名单——登录（/auth/login）、注册（/auth/register）、查询帖子列表（GET /posts）、查询帖子详情（GET /posts/{id}）、静态文件访问（/uploads/**）、Swagger 文档（/swagger-ui/**）、健康检查（/health、/actuator/**）等接口无需认证即可访问。其余接口（创建帖子、点赞、收藏、评论、修改资料等）均要求携带有效的 JWT Token。
...
**传输安全层面：**开发环境中使用 HTTP 协议（localhost），生产环境中（Vercel + Render 部署）强制升级为 HTTPS 协议。跨域资源共享（CORS）配置限制了允许访问 API 的域名源——通过 `allowedOriginPatterns("*")` 在开发阶段开放所有来源，但明确标注了生产环境需要收紧为具体的生产域名。
...
文件上传安全通过多重校验实现。首先是文件类型白名单——仅允许 `image/jpeg`、`image/png`、`image/gif`、`image/webp`、`image/svg+xml`、`video/mp4`、`video/webm`、`video/ogg` 八种 MIME 类型。其次是文件扩展名白名单——通过正则表达式 `\.(jpg|jpeg|png|gif|webp|svg|mp4|webm|ogg)$` 校验文件名后缀。第三是文件大小限制——单文件最大 50MB。第四是路径遍历攻击防护——通过 `targetLocation.normalize().startsWith(fileStorageLocation)` 检查确保上传的文件路径不会逃逸出 uploads 目录。
...
**自动化测试覆盖**方面，项目的 CI/CD 流水线（GitHub Actions）在每次代码推送和 Pull Request 时自动运行全部后端测试。测试结果和覆盖率报告（通过 JaCoCo 插件生成）上传到 GitHub Actions Artifacts，供团队成员查看。目前后端的核心 Service 层测试覆盖率达到约 70%，Controller 层覆盖率达到约 60%。
...
**前端关键依赖：**vue@3.4+（核心框架）、vite@5.x（构建工具）、vant@4.x（移动端 UI 组件库）、@vant/auto-import-resolver（Vant 组件自动导入解析器）、unplugin-vue-components（组件按需引入插件）、vitest@1.x（单元测试框架）、@vue/test-utils（Vue 组件测试工具）、jsdom（浏览器环境模拟）、@vitejs/plugin-vue（Vite 的 Vue 3 支持插件）。

**Vant 4 按需引入配置：**通过 `unplugin-vue-components` 插件 + `VantResolver` 实现组件自动按需引入。开发者在 `.vue` 模板中直接使用 `<van-button>`、`<van-tabbar>`、`<van-popup>` 等 Vant 组件，构建时插件自动检测模板中使用的组件名称，只将实际使用的组件代码打包到最终产物中，未使用的组件不会被打包——这一 Tree Shaking 机制有效控制了生产构建的 JavaScript 体积。

**构建优化配置：**Vite 5 基于 Rollup 进行生产构建，自动进行代码分割——第三方依赖（Vant、Vue）打包为 vendor chunk，组件代码按需拆分为独立 chunk。CSS 通过 Vite 内置的 CSS 处理管线自动提取和压缩。生产构建产物中，JS 文件启用 Brotli 压缩（配合 Nginx/Vercel），CSS 文件启用 Minification（删除空白和注释），图片资源自动通过 imagemin 优化。
...
**登录注册实现：**`AuthModal.vue` 组件实现了登录和注册两种模式的切换。组件使用 Vant 的 `van-field` 组件渲染表单输入框，内置表单验证逻辑（用户名不能为空、密码长度不小于 6 位）。登录按钮点击后，调用 `api/index.js` 中的 `login(username, password)` 函数向 `POST /auth/login` 发送请求。成功登录后，后端返回的用户信息（包括 JWT Token）被存储到 `localStorage` 中，通过 `App.vue` 的 `provide` 注入机制分发给所有后代组件。
...
**多图上传状态：**使用 `ref([])` 管理 9 个格子的上传状态。每个格子有两种状态——空状态（显示"+"图标，可触发文件选择）和已上传状态（显示图片缩略图 + 右上角删除按钮）。图片选择通过隐藏的 `<input type="file" multiple accept="image/*">` 元素实现，监听 `change` 事件获取选中的文件列表。上传逻辑采用"逐个上传、顺序确认"策略——对每个文件依次调用 `POST /posts/upload` 接口，前一个文件上传成功后再上传下一个，确保所有文件都成功上传后再进入下一步。这种策略虽然比并发上传慢，但避免了"部分成功部分失败"的复杂错误处理场景。
...
**AI 内容创作助手：**发布页面提供了 AI 辅助写作功能。用户在正文区域输入一个简短的主题或关键词（如"红烧肉做法"），点击"AI 创作"按钮后，后端调用 DeepSeek API，根据用户输入的主题生成一篇完整的小红书风格的笔记内容（包括标题建议、正文叙述、要点总结和合适的语气风格）。生成的草稿自动填充到正文区域，用户可以在 AI 生成的基础上进行修改和个性化调整，大幅降低内容创作的门槛。
...
**浏览器兼容性处理：**Web Speech API 在不同浏览器上的支持度不一。组件在初始化时进行兼容性检查——`window.SpeechRecognition || window.webkitSpeechRecognition`，如果两个对象都不存在（如 Firefox 部分版本），则隐藏麦克风按钮并回退到纯文本输入搜索模式，确保搜索功能在所有浏览器上的可用性。
...
Vite 5 基于 Rollup 的构建系统自动对项目进行代码分割。生产构建中，Vue 3 核心库（约 126KB gzipped）和 Vant 4 组件库（约 80KB gzipped，按需引入后进一步减小）被自动分离为独立的 vendor chunk。应用自己的组件代码按路由/视图拆分为多个异步 chunk，浏览器在首次加载时只下载渲染首屏所需的代码，其他视图的代码在用户切换时才按需加载。
...
| API 文档 | SpringDoc OpenAPI | 2.x | Swagger 文档自动生成 |
...
认证链路：POST /auth/login → AuthController → AuthService → UserRepository.findByUsername → BCrypt.matches 验证 → JwtUtil.generateToken 生成 JWT（含 sub/username/iat/exp 声明，HS256 签名，24h 有效期）→ 返回 Token + 用户信息。
...
SecurityConfig 白名单：/auth/login、/auth/register、GET /posts、/uploads/**、/health、/swagger-ui/** 等公开端点无需认证。

选择无状态 JWT 而非 Session 的理由：前后端分离架构中 JWT 无需服务端维护会话状态，水平扩展时各实例独立验证 Token，天然适配分布式部署。

### 7.2.2 帖子管理服务

PostService 是业务逻辑最集中的 Service。getAllPosts 按 created_at 倒序返回（Eager Fetch 一次性 JOIN 作者信息），支持 title 关键词模糊搜索和 tag 分类筛选。toggleLike/toggleCollect 采用 Toggle 设计——存在则 delete，不存在则 create，利用 UNIQUE 约束防重复。createPost 校验作者存在性后保存实体，imageUrls 序列化为 JSON 存储。

### 7.2.3 AI 服务实现

AIService.generateSummary(title, content) 四步流程：1) 构建 System + User 双层 Prompt；2) HTTP POST 调用 DeepSeek API（model: deepseek-v4-flash, temperature: 0.3, max_tokens: 200）；3) 解析 choices[0].message.content；4) 缓存到 posts.ai_summary 字段。命中缓存时响应时间从 ~3s 降至 <10ms。另三个 AI 方法：recommendTags（标签推荐）、generateContent（内容创作）、answerQuestion（帖子问答）。

### 7.2.4 评论系统实现

评论系统是项目中最为复杂的业务模块之一，涉及多表关联查询、嵌套结构组装和通知联动。PostService.createComment 方法采用"两层扁平化"嵌套设计——所有子回复的 parent_id 统一指向顶级根评论。具体实现为：当用户回复某条本身已是子评论的评论时，新回复的 parent_id 不会被设为被回复评论的 ID，而是被设为该评论的根评论 ID。这确保前端渲染评论树时只需一层 subComments 嵌套，简化递归渲染逻辑。评论创建时自动设置 replyToUser 和 replyToCommentId 字段，并同步触发 NotificationService 创建通知。删除采用软删除策略（不级联删除子回复），维护评论树完整性。


### 7.2.5 通知服务实现

NotificationService 事件驱动：点赞/收藏/评论/关注发生时同步创建通知记录（type: LIKE/COLLECTION/COMMENT/FOLLOW）。getNotificationsByType 按 userId + type 筛选，created_at 倒序，前端按 Tab 分段展示并进行已读标记。
...
| 文件上传 | 无限制 | 50MB 上限 + 流式写入 | FileController 校验 |
...
**代码生成**：GitHub Copilot 辅助 Vue 组件模板（Vant 属性自动补全）和 Spring Controller 标准 CRUD 实现，约占整体代码量 25-30%。**文档编写**：spec.md、backend.md、frontend.md、api.md、report.md 等文档由 AI 辅助完成初稿和格式化。**代码审查**：AI 预审查识别未使用 import、潜在 NPE、安全漏洞。**调试**：AI 分析完整错误上下文推断根因。

## 8.2 AI 辅助故障排查典型案例

**图片文件 404 问题**：现象为前端所有图片返回 404，磁盘文件确实存在。提供完整上下文（WebConfig 源码、FileController 源码、application.properties、Vite proxy 配置）给 AI 分析。AI 诊断根因为 FileController 的 @RequestMapping("/uploads") 拦截了所有 /uploads/** 请求，Spring MVC 优先匹配 Controller 且不自动回退到资源处理器。解决方案为改 Controller 路径为 /files。修改后图片立即 200 OK。

## 8.3 AI 功能集成

四项 AI 功能通过 DeepSeek API 实现：**AI 智能总结**（100 字摘要 + 数据库持久化缓存）、**AI 标签推荐**（根据内容推荐 1-3 个分类标签）、**AI 内容创作助手**（根据主题生成小红书风格完整笔记）、**AI 帖子问答**（基于帖子内容的自然语言问答）。核心技术：Prompt 工程（System + User 双层模板）+ 缓存策略（永久有效，笔记内容不变则摘要不变）。
...
**身份认证**：JWT HS256 签名 + 24h 过期 + application-secrets.properties 隔离密钥管理。**输入验证**：JPA PreparedStatement 参数绑定防 SQL 注入 + Vue 模板引擎 {{ }} 自动 HTML 转义防 XSS + DTO Bean Validation 注解。**密码存储**：BCrypt(workFactor=10) + 自动随机盐值。**文件安全**：MIME 白名单（8 种类型）+ 扩展名正则校验 + 50MB 大小限制 + 路径遍历防护。**CORS**：开发全开标注收紧，生产限定具体域名。**安全头**：Spring Security 默认添加 X-Content-Type-Options、X-Frame-Options、X-XSS-Protection 等。

## 9.3 安全审计

前端 npm audit 检查 NPM 依赖漏洞，后端 OWASP Dependency-Check Maven 插件在构建时检查全部依赖的 CVE 记录。CI/CD 流水线自动执行安全扫描，发现高危漏洞（Critical/High）标记 CI 失败。
...
**前端**：Vitest + @vue/test-utils + jsdom，16 个测试文件，215 个用例 100% 通过。语句覆盖率 81.80%，分支覆盖率 82.20%。API 层 100%（Mock Fetch），组件层 86.51%（核心交互组件全覆盖），Composables 层 63.15%。
...
第一条测试用例（用户注册成功场景）：使用 MockMvc 模拟 `POST /auth/register` 请求，请求体为 `{"username": "newuser", "password": "123456", "nickname": "新用户"}`。Mock UserRepository 的 `existsByUsername` 返回 false（用户名未注册），`save` 返回包含 ID=1 的新 User 对象。预期 HTTP 状态码为 200 OK，响应体包含 JWT Token 字段，UserRepository.save() 被调用一次。
...
第三条测试用例（点赞 Toggle 逻辑）：单元测试 PostService 的 toggleLike 方法。第一阶段——调用 toggleLike(userId=1, postId=1)，Mock LikeRepository 的 findByUserAndPost 返回 Optional.empty()（尚未点赞），预期 likeRepository.save() 被调用一次（创建点赞），返回 isLiked=true。第二阶段——再次调用 toggleLike(userId=1, postId=1)，Mock 返回 Optional.of(existingLike)（已点赞），预期 likeRepository.delete() 被调用一次（取消点赞），返回 isLiked=false。

第四条测试用例（AI 总结缓存逻辑）：单元测试 AIService 的 generateSummary 方法。第一阶段——帖子 aiSummary 字段为 null，Mock RestTemplate 返回 DeepSeek API 的 JSON 响应（含 choices[0].message.content），预期返回 AI 生成内容且调用了一次 RestTemplate。第二阶段——帖子 aiSummary 已有缓存内容（"这道菜做法简单..."），预期直接返回缓存内容，RestTemplate 未被调用（零次 HTTP 请求）。

第五条测试用例（评论嵌套结构）：单元测试 PostService 的 createComment 方法。场景——用户 B 回复用户 A 的顶级评论（parentId = A 的评论 ID）。预期创建的 Comment 实体的 parentId 被设置为 A 的评论 ID，replyToUser 被设置为用户 A，replyToCommentId 被设置为 A 的评论 ID。同时验证 notificationService.create() 被调用一次（向用户 A 发送通知）。

## 10.3 测试结果汇总

| 测试类型 | 框架 | 用例数 | 通过率 | 覆盖率 |
|---------|------|--------|--------|--------|
| 前端单元测试 | Vitest + vue/test-utils | 215 | 100% | 81.80% 语句 |
...
GitHub Actions（0 配置 + 2000min/mo 免费 + 丰富生态）。.github/workflows/ci.yml 在 Push/PR 到 main 时触发。两个并行 Job：Backend（JDK 17 + mvn clean verify + JaCoCo）和 Frontend（Node 22 + npm ci + ESLint --max-warnings 0 + vitest run --coverage + Codecov 上传）。

## 11.2 分支保护与质量门禁

main 分支保护：CI 全绿 + >=1 人 Code Review Approve + 禁止 Force Push + 分支同步。前端覆盖率 >=60%（实际 81.80%），ESLint 零警告。自动安全扫描阻止高危漏洞合并。
...
三种方案：**本地开发**（mvn spring-boot:run + npm run dev，Vite 代理 /api 到 :8080）、**Docker 本地**（docker-compose up --build，三容器：backend + frontend(Nginx) + host.docker.internal:MySQL）、**云平台混合**（Vercel 前端 CDN + Render 后端 Docker + 本地 MySQL）。
...
克隆仓库 → 创建 xiaohongshu 数据库 + 执行 schema.sql → 复制 secrets.example 为 secrets.properties 并填入真实值 → 启动后端 → 启动前端 → 浏览器访问 localhost:5173。

## 12.4 环境配置

敏感配置通过 .gitignore + .example 模板隔离。Spring Boot 配置优先级：命令行参数 > 环境变量 > application-secrets.properties > application.properties。生产环境通过 Render 环境变量面板注入 JWT Secret 和 API Key。
...
**Vercel（前端）**：全球 100+ CDN 节点，GitHub 推送即自动部署，免费 100GB/月带宽，rewrites 代理 API，自动 HTTPS。**Render（后端）**：Docker 原生支持，免费 512MB RAM + 0.1 CPU + 1GB 存储，自动 HTTPS，空闲 15min 后休眠（冷启动 ~30s）。
...
| 后端托管 | Render | Spring Boot Docker 容器 | Free 512MB |
| 数据库 | self-hosted MySQL | 主数据存储 | 本地宿主机 |
...
全部免费层级，月度总费用 0 元。Vercel: 100GB 带宽 + 6000min 构建/月。Render: 512MB RAM + 0.1 vCPU + 1GB 持久化卷。Codecov: 公开仓库无限覆盖率报告。
...
当前通过 Logback JSON 日志记录所有异常（含 Stack Trace、请求路径、时间戳）。通过 docker-compose logs 或 Render Dashboard 实时查看。未来计划集成 Sentry Spring Boot Starter (7.x) 实现错误自动聚合、去重和告警。
...
| Vant 体积 | 全量引入 ~200KB | 按需引入 ~80KB | unplugin-vue-components Tree Shaking |
...
**首页瀑布流**：搜索栏（集成语音搜索麦克风）+ 分类标签横滑栏（10 个标签）+ 瀑布流卡片（3:4 封面大图 + 标题 + 作者信息 + 点赞数）。桌面端 240px 左侧边栏导航，移动端 Vant TabBar（首页/发布/通知/我）。**笔记详情**：桌面 1000x680px 弹窗左右布局（左图右文），移动端全屏上下布局。多图支持左右箭头轮播，视频使用 HTML5 player。AI 总结按钮点击后 2-3 秒生成 100 字摘要。**发布页**：3x3 图片上传网格 + 标题输入 + 正文编辑 + 标签选择 + AI 推荐标签 + AI 创作助手。**通知中心**：类型筛选 + 未读高亮 + 相对时间 + 点击跳转。**个人主页**：大头像 + 统计栏 + 内容 Tab（笔记/点赞/收藏）。**登录注册**：Vant van-field 表单 + 测试账号提示。
...
Essencity 历时 15 周，从零构建了完整仿小红书社交平台。技术覆盖 Vue 3 + Spring Boot 3 全栈、40 余项功能（含 4 项 AI）、215+ 前端测试 + ~70 后端测试、Docker 容器化 + 双云平台部署。总代码约 45,000+ 行（含测试和配置）。

## 17.2 技术收获

**林忠阳**：Spring Boot 3 分层架构与 DI、JPA ORM 关联映射与查询优化、JWT + Spring Security 认证体系、DeepSeek API Prompt 工程与缓存、Docker 多阶段构建与 Compose 编排、Actuator + Micrometer 可观测性、Vercel + Render 云部署。**林烨澄（前端开发 + 测试 + 安全）：**深入掌握了 Vue 3 Composition API 的组件设计理念——通过 setup() 函数和响应式 API（ref、reactive、computed、watch）将组件的逻辑和模板清晰分离，实现比 Options API 更灵活的逻辑复用。熟练掌握了 Vant 4 移动端组件库的按需引入机制（unplugin-vue-components + VantResolver）和主题定制（通过 CSS 变量覆盖 Vant 的默认 Design Token，适配品牌红色主题）。精通了 Vitest + @vue/test-utils + jsdom 的前端测试工具链，能够编写覆盖组件渲染、用户交互、API Mock 和边界条件的完整测试用例，并理解覆盖率报告的各项指标含义。深入理解了 Web Speech API 的浏览器兼容性差异——Chrome 完全支持、Safari 部分支持、Firefox 不支持——并设计实现了优雅降级策略（不支持时隐藏语音按钮，回退纯文本搜索）。熟练掌握了响应式设计的完整工作流——从 320px 到 4K 的逐级断点规划、CSS Media Query 的布局切换、Vant 组件的移动端适配以及 env(safe-area-inset-bottom) 的刘海屏适配。深入理解了文件上传安全的完整防线——MIME 类型白名单、文件扩展名正则校验、文件大小上限、路径遍历防护（normalize + startsWith）。**陈熠恒**：GitHub Actions Workflow YAML 配置与并行 Job 设计、ESLint 浏览器/测试环境差异化配置、Vitest CI 模式与覆盖率报告生成、Codecov 覆盖率追踪、npm audit 安全扫描与 CI 质量门禁。

## 17.3 问题与反思

六大关键教训：

**教训一：Spring Boot Controller 路径与资源处理器优先级冲突。**FileController 的 @RequestMapping("/uploads") 拦截所有 /uploads/** 请求，Spring MVC 优先匹配 Controller 且不自动回退。教训：设计阶段即区分 API 路径和静态资源路径前缀。
...
**教训六：多部署模式下 URL 规范化复杂性。**本地/Docker/Vercel+Render 三种部署导致同一资源多种 URL 前缀（/uploads/、/api/uploads/、http://localhost:8080/api/uploads/）。更好的方案是后端 API 统一返回完整绝对 URL。
...
[24] Pandoc - Universal Document Converter. https://pandoc.org/

[25] 火山引擎豆包大模型 API 文档. https://www.volcengine.com/docs/82379

[26] Spring Framework Reference Documentation. https://docs.spring.io/spring-framework/reference/

[27] Conventional Commits Specification. https://www.conventionalcommits.org/

[28] GitHub Flow Guide. https://docs.github.com/en/get-started/using-github/github-flow

[29] JaCoCo Java Code Coverage. https://www.jacoco.org/

[30] IntersectionObserver API - MDN Web Docs. https://developer.mozilla.org/en-US/docs/Web/API/Intersection_Observer_API
...
| SpringDoc OpenAPI | 2.x | Swagger/OpenAPI 文档 | https://springdoc.org |
...
| unplugin-vue-components | 0.x | 组件自动导入 | https://github.com/unplugin/unplugin-vue-components |
...
│   │   │   ├── AuthModal.vue（登录认证）
│   │   │   ├── AiSummary.vue、VoiceSearch.vue（AI/语音）
│   │   ├── composables/  # useSpeech.js 语音识别
│   │   ├── __tests__/    # 16 个测试文件（215 用例）
│   │   ├── main.js / style.css
│   ├── index.html
...
│   │   ├── service/         # AuthService, PostService, AIService, FileService, NotificationService
│   │   ├── repository/      # User, Post, Comment, Like, Collection, Follow, Notification Repositories
...
│   ├── uploads/             # 文件存储（test/ 目录 15 张测试图片 + default-avatar.png）
│   ├── pom.xml / Dockerfile
│
